<?php
/**
 * Plugin Name: ATR Mobile Detect
 * Plugin URI: http://atarimtr.com
 * Description: WP plugin - Detect mobile devices - Based on https://github.com/serbanghita/Mobile-Detect. Use: $mobile_detect = new ATR_Mobile_Detect; if ( $mobile_detect->isMobile() ) {	Do something }
 * Version: 1.0.0
 * Author: Yehuda Tiram
 * Author URI: http://atarimtr.com
 * All right reserved to AtarimTr LTD
 * Text Domain:       atr-mobile-detect
...
class ATR_Mobile_Detect extends Mobile_Detect {
	public function get_browser_name_ver(){
		$str = $this->userAgent; // Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.108 Safari/537.36
		$browser_arr = array('Chrome/', 'Firefox/');
		$browser_ver = '';
		$current_browser_name = '';
		$current_browser_ver = '';
		foreach ($browser_arr as $value) {
			if (strpos($str, $value) !== false) {
				$browser_ver = stristr($str, $value);
				$current_browser_name = explode("/", $browser_ver);
				$current_browser_ver = explode(".", $current_browser_name[1]);
				break;
			}			
		}	
		$browser_ver_arr = []; 
		if ( ( $current_browser_name != NULL ) && ( $current_browser_ver != NULL ) ){
			$browser_ver_arr[0] = $current_browser_name[0];
			$browser_ver_arr[1] = $current_browser_ver[0];			
		}
		else {
			$browser_ver_arr[0] = 'no_name';
			$browser_ver_arr[1] = 'no_version';				
		}

		return $browser_ver_arr;
	}	
}
